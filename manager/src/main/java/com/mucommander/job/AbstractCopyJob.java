...
/**
 * This class is the parent class of {@link com.mucommander.job.CopyJob} and
 * {@link com.mucommander.job.MoveJob} and allows them to share methods and
 * fields.
 *
...
public abstract class AbstractCopyJob extends TransferFileJob {
	 ...
	 *            dialog which shows this job's progress
	 * @param mainFrame
	 *            mainFrame this job has been triggered by
	 * @param files
	 *            files which are going to be copied
	 * @param destFolder
	 *            destination folder where the files will be copied
	 * @param newName
	 *            the new filename in the destination folder, can be
	 *            <code>null</code> in which case the original filename will be
	 *            used.
	 * @param fileExistsAction
	 *            default action to be performed when a file already exists in
	 *            the destination, see
	 *            {@link com.mucommander.ui.dialog.file.FileCollisionDialog} for
	 *            allowed values
	 */
	public AbstractCopyJob(ProgressDialog progressDialog, MainFrame mainFrame,
			FileSet files, AbstractFile destFolder, String newName,
	...
	/**
	 * Creates a destination file given a destination folder and a new file
	 * name.
	 * 
	 * @param destFolder
	...
	protected AbstractFile createDestinationFile(AbstractFile destFolder,
		...
		do { // Loop for retry
			try {
				...
				int ret = showErrorDialog(errorDialogTitle,
						Translator.get("cannot_write_file", destFileName));
				// Retry loops
				if (ret == RETRY_ACTION)
					continue;
	...
	/**
	 * Checks if there is a file collision (file exists in the destination). If
	 * there is no collision this method returns destFile. If there is a
	 * collision this method returns:
	 * <ul>
	 ...
	 * @param destFile
	 *            a destination file
	 * @param allowCaseVariation
	 *            if true,
	 * @return destFile the new destination file
	 */
	protected AbstractFile checkForCollision(AbstractFile file,
			AbstractFile destFolder, AbstractFile destFile,
			boolean allowCaseVariation) {
		append = false;
		while (true) {
			// Check for file collisions (file exists in the destination,
			// destination subfolder of source, ...)
			// if a default action hasn't been specified
			int collision = FileCollisionChecker.checkForCollision(file,
					destFile);

			// If allowCaseVariation is true and both files are equal, test if
			// the destination filename is a variation
			// of the original filename with a different case. If that is the
			// case, do not warn about the source and
			// destination being the same.
			if (allowCaseVariation
					&& collision == FileCollisionChecker.SAME_SOURCE_AND_DESTINATION) {
				String sourceFileName = file.getName();
				String destFileName = destFile.getName();
				if (sourceFileName.equalsIgnoreCase(destFileName)
						&& !sourceFileName.equals(destFileName))
					break;
			}

			// Handle collision, asking the user what to do or using a default
			// action to resolve the collision
			if (collision != FileCollisionChecker.NO_COLLOSION) {
				int choice;
				// Use default action if one has been set, if not show up a
				// dialog
				if (defaultFileExistsAction == FileCollisionDialog.ASK_ACTION) {
					FileCollisionDialog dialog = new FileCollisionDialog(
							getProgressDialog(), getMainFrame(), collision,
							file, destFile, true, true);
					choice = waitForUserResponse(dialog);
					// If 'apply to all' was selected, this choice will be used
					// for any other files (user will not be asked again)
				...
				// Overwrite file if destination is older
					...
					setPaused(true);
					FileCollisionRenameDialog dlg = new FileCollisionRenameDialog(
							getMainFrame(), destFile);
	...
	/**
	 * Optimizes the given writable archive file and notifies the user in case
	 * of an error.
	 *
	 * @param rwArchiveFile
	 *            the writable archive file to optimize
	 */
	protected void optimizeArchive(AbstractRWArchiveFile rwArchiveFile) {
		...
		while (true) {
			try {
				...
				if (showErrorDialog(errorDialogTitle, Translator.get(
						"error_while_optimizing_archive",
						rwArchiveFile.getName())) == RETRY_ACTION)
					continue;
